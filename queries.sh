...
echo "$($PSQL "SELECT name 
               FROM games LEFT JOIN teams on games.winner_id = teams.team_id
               where year = 2018 and round = 'Final' ")"


echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT DISTINCT(name) as teamName
               FROM teams INNER JOIN games ON teams.team_id = games.winner_id 
